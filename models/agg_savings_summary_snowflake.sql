...
-- dbt run --select +agg_savings_summary_snowflake --vars '{"summary_start_date": "2025-12-04", "summary_end_date": "2025-12-11"}'
...
ROUND(SUM(reuse_count) / SUM(reuse_count + execute_count), 2) * 100 as weekly_perc_reused_models,
 --model costs
SUM(estimated_cost_saved_usd) as weekly_reused_cost_savings,
SUM(estimated_cost_spent_usd) as weekly_cost_spent,
ROUND(SUM(estimated_cost_saved_usd) / SUM(estimated_cost_saved_usd + estimated_cost_spent_usd), 2) * 100 as weekly_perc_cost_savings
