...
# Test fixture
export DBT_CLOUD_RUN_REASON="test' single quote and special characters: #$%^&*()_+-=[]{};:,.<>?\`. literal double-quote: \", literal backslash + single quote: \\', literal backslash: \\, literal backslash + backslash: \\\\ and finally "$'\n'
...
run_dbt_command() {
    ...
    # Construct vars flag only if vars_param is not empty
    local vars_flag=""
    if [ -n "$vars_param" ]; then
        vars_flag="--vars \"$vars_param\""
    fi
    ...
    case $command in
        ...
        "compile")
            ...
            ;;
        "run")
            $resolved_command run --target $adapter --profiles-dir .. $vars_flag
            ;;
        "build")
